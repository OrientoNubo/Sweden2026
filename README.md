...
- **刪去法選景點**:預載兩國 500+ 個大小景點(繁中說明、分類、圖片),刪掉不想去的,留下的就是行程候選
- 整理模式(cluster 瀏覽/篩選/搜尋/收藏/回收站)與行程模式(9/4–9/15 按日指派、每日一色、順序折線)
- 雙擊(手機長按)地圖新增自訂標注,說明卡可帶圖片(可上傳,存瀏覽器 IndexedDB)
- 自由繪製路線:逐點畫線、編輯節點、命名調色、距離計算
